...
class RBM():
  ...
  def sample_h(self, x): #x - values from visible nodes
    wx = torch.mm(x, self.W.t())
    activation = wx + self.a.expand_as(wx)
    p_h_given_v = torch.sigmoid(activation)
    return p_h_given_v, torch.bernoulli(p_h_given_v)
    #p_h_given_v - probabilities, that values from visible nodes will activate hidden nodes
    #torch.bernoulli(p_h_given_v) - vector of enabled and disabled hidden nodes (1 or 0)

  def sample_v(self, y): #y - values from hidden nodes
...
for epoch in range(1, nb_epoch + 1):
  ...
  for id_user in range(0, nb_users - batch_size, batch_size):
    ...
    ph0,_ = rbm.sample_h(v0) #get first array value
    for k in range(10):
      _,hk = rbm.sample_h(vk) #get second array value
      _,vk = rbm.sample_v(hk)
      vk[v0<0] = v0[v0<0] #we reset values for -1 (not rated films)
